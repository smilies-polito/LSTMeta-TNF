...
parser.add_argument('--window_size', type=int, default=12, help='Length of the window in number of steps')
...
EXP_DIR = os.path.join("experiments", "tnfvector")
...
if 'mac' in platform.platform():
    ...
    DATA_DIR_2 = "/Users/marco/phd/meta-model-physiboss/metamodel/data_2D/"
...
data2 = pd.read_csv(os.path.join(DATA_DIR_2, "data.csv"))
...
if TUMOR_RADIUS == 100:
    data2['tumor_radius'] = 100
    data2['index_input_params'] += (data["index_input_params"].max()+1)
    data = pd.concat([data, data2])

data['time_add_tnf_int'] = data['time_add_tnf'].map(lambda x: x//TIMESTEP+int(x%TIMESTEP > TIMESTEP/2)).astype(int)
data['duration_add_tnf_int'] = data['duration_add_tnf'].map(lambda x: x//TIMESTEP+int(x%TIMESTEP > TIMESTEP/2)).astype(int)

data = data[data['concentration_tnf'] > 0.1]
data = data[data['duration_add_tnf_int'] > 0]

# Split train and test
index_input_params = data["index_input_params"].unique()

train_size = int(0.87 * len(index_input_params))
...
def generate_tnf_vector(x):
    a = np.zeros(len(x['alive']), dtype=np.float32)
    t_every = x['time_add_tnf_int']
    t_len = x['duration_add_tnf_int']
    tnf_conc = x['concentration_tnf']
    for x in range(t_len):
        a[t_every+x::t_every] = tnf_conc
    return a

data['tnf_vector'] = data.apply(generate_tnf_vector, axis=1)
...
INPUT_PARAMS_NORM_FACTOR = data_train[INPUT_PARAMS_ORDER].max().tolist()
...
tnf_vector_train = np.array(data_train['tnf_vector'].to_list())
tnf_vector_train = tnf_vector_train[shuffled_indices]
tnf_vector_train = np.expand_dims(np.reshape(
    tnf_vector_train, (-1, BATCH_SIZE_TRAIN, tnf_vector_train.shape[1])
), -1)
print(f"TNF vector train {tnf_vector_train.shape}")
...
tnf_vector_test = np.array(data_test['tnf_vector'].to_list())
tnf_vector_test = tnf_vector_test[shuffled_indices]
tnf_vector_test = np.expand_dims(np.reshape(
    tnf_vector_test, (-1, BATCH_SIZE_TEST, tnf_vector_test.shape[1])
), -1)
print(f"TNF vector test {tnf_vector_test.shape}")
...
dl_train = torch.utils.data.DataLoader(
    WindowedPredictionDatasetContinuous(
        input_params_train, cell_states_train, WINDOW_SIZE, tnf_vector_train
    ), shuffle=False
)
dl_test = torch.utils.data.DataLoader(
    WindowedPredictionDatasetContinuous(
        input_params_test, cell_states_test, 2, tnf_vector_test
    ), shuffle=False
)
...
for d in dl_train:
    inp, _, cells, tnf, labels = d
    for i, c, t, l in zip(inp.squeeze(dim=0), cells.squeeze(dim=0), tnf.squeeze(dim=0), labels.squeeze(dim=0)):
        print(i.shape, c.shape, t.shape, l.shape)
...
MODEL_NAME = f'{ARCH.upper()}_radius{TUMOR_RADIUS}_s{SEED}_ss{SUBSAMPLE_EVERY}_w{WINDOW_SIZE}'+\
...
if WANDB_LOG:
    wandb.init(
        project=f"metamodel_gridsearch",
        config={
...
from utils.training import train_epoch_bptt_tnfvector, test_epoch_bptt_tnfvector
...
for epoch in range(EPOCHS):
    ...
    model.train()
    avg_loss = train_epoch_bptt_tnfvector(
        model,
        optimizer=optimizer,
        loss_fn=loss_fn,
        device=device,
